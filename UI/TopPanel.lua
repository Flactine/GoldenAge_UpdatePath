	...
	if ( eLocalPlayer ~= -1 and owner == eLocalPlayer ) then
		...
		for entry in GameInfo.WMDs() do
			...
			elseif (entry.WeaponType == "WMD_CASTLE_BRAVO") then
				local count = playerWMDs:GetWeaponCount(entry.Index);
				if (count > 0) then
					Controls.CestleNuclearDevices:SetHide(false);
					Controls.CestleNuclearDeviceCount:SetText(count);
				else
					Controls.CestleNuclearDevices:SetHide(true);
				end

			elseif (entry.WeaponType == "WMD_KUZKINE_MET") then
				local count = playerWMDs:GetWeaponCount(entry.Index);
				if (count > 0) then
					Controls.KuzkeNuclearDevices:SetHide(false);
					Controls.KuzkeNuclearDeviceCount:SetText(count);
				else
					Controls.KuzkeNuclearDevices:SetHide(true);
				end

			elseif (entry.WeaponType == "WMD_R36_ORB") then
				local count = playerWMDs:GetWeaponCount(entry.Index);
				if (count > 0) then
					Controls.R36OrbNuclearDevices:SetHide(false);
					Controls.R36OrbNuclearDeviceCount:SetText(count);
				else
					Controls.R36OrbNuclearDevices:SetHide(true);
				end
			elseif (entry.WeaponType == "WMD_MISSILE_BASE") then
				local count = playerWMDs:GetWeaponCount(entry.Index);
				if (count > 0) then
					Controls.BASEMISSILEDevices:SetHide(false);
					Controls.BASEMISSILEDeviceCount:SetText(count);
				else
					Controls.BASEMISSILEDevices:SetHide(true);
				end	
			end
		end
