...
function CityBanner:CreateAerodromeBanner()
	-- Set the appropriate instance factory (Scapeh don't know shit stole that file from Raubak) for this flag...
...
function CityBanner:CreateWMDBanner()
	...
	-- GA:WMD
	local eR36_ORB = GameInfo.WMDs["WMD_R36_ORB"].Index;
	self.m_Instance.R36OrbNukeBombButton:RegisterCallback( Mouse.eLClick, OnICBMStrikeButtonClick );
	self.m_Instance.R36OrbNukeBombButton:SetVoid1(plotID);
	self.m_Instance.R36OrbNukeBombButton:SetVoid2(eR36_ORB);

	local eMISSILE_BASE = GameInfo.WMDs["WMD_MISSILE_BASE"].Index;
	self.m_Instance.MISSILE_BASEBombButton:RegisterCallback( Mouse.eLClick, OnICBMStrikeButtonClick );
	self.m_Instance.MISSILE_BASEBombButton:SetVoid1(plotID);
	self.m_Instance.MISSILE_BASEBombButton:SetVoid2(eMISSILE_BASE);
	
	local eCASTLE_BRAVO = GameInfo.WMDs["WMD_CASTLE_BRAVO"].Index;
	self.m_Instance.CASTLE_BRAVOBombButton:RegisterCallback( Mouse.eLClick, OnICBMStrikeButtonClick );
	self.m_Instance.CASTLE_BRAVOBombButton:SetVoid1(plotID);
	self.m_Instance.CASTLE_BRAVOBombButton:SetVoid2(eCASTLE_BRAVO);
	
	local eKUZKINE_MET = GameInfo.WMDs["WMD_KUZKINE_MET"].Index;
	self.m_Instance.KUZKINE_METBombButton:RegisterCallback( Mouse.eLClick, OnICBMStrikeButtonClick );
	self.m_Instance.KUZKINE_METBombButton:SetVoid1(plotID);
	self.m_Instance.KUZKINE_METBombButton:SetVoid2(eKUZKINE_MET);
end
...
function CityBanner:UpdateWMDBanner()
	...
	for entry in GameInfo.WMDs() do
		...
		-- GA:WMD
		elseif (entry.WeaponType == "WMD_R36_ORB") then
			local count = playerWMDs:GetWeaponCount(entry.Index);
			if (count > 0) then
				-- Player has R36Orb bombs
				self.m_Instance.R36OrbNukeCountLabel:SetText(count);
				self.m_Instance.R36OrbNukeBombButtonBackground:SetHide(false);
				-- Check if we're able to fire
				local bSiloCanFire:boolean = false;
				if( pCity ~= nil ) then
					local tParameters = {};
				...
				-- Update button state and tooltip
				if( bSiloCanFire) then
					self.m_Instance.R36OrbNukeBombButton:SetDisabled(false);
					self.m_Instance.R36OrbNukeBombButton:SetToolTipString(Locale.Lookup("LOC_CITY_BANNER_R36_ORB_STRIKE_CAPABLE"));
				else
					self.m_Instance.R36OrbNukeBombButton:SetDisabled(true);
					self.m_Instance.R36OrbNukeBombButton:SetToolTipString(Locale.Lookup("LOC_CITY_BANNER_WEAPON_UNAVAILABLE"));
				end
			else
				-- Player does not have R36Orb bombs
				self.m_Instance.R36OrbNukeCountLabel:SetText("0");
				self.m_Instance.R36OrbNukeBombButtonBackground:SetHide(true);
			end	
		elseif (entry.WeaponType == "WMD_MISSILE_BASE") then
			local count = playerWMDs:GetWeaponCount(entry.Index);
			if (count > 0) then
				self.m_Instance.MISSILE_BASECountLabel:SetText(count);
				self.m_Instance.MISSILE_BASEBombButtonBackground:SetHide(false);
				-- Check if we're able to fire
				local bSiloCanFire:boolean = false;
				if( pCity ~= nil ) then
					local tParameters = {};
				...
				-- Update button state and tooltip
				if( bSiloCanFire) then
					self.m_Instance.MISSILE_BASEBombButton:SetDisabled(false);
					self.m_Instance.MISSILE_BASEBombButton:SetToolTipString(Locale.Lookup("LOC_WMD_MISSILE_BASE_NAME"));
				else
					self.m_Instance.MISSILE_BASEBombButton:SetDisabled(true);
					self.m_Instance.MISSILE_BASEBombButton:SetToolTipString(Locale.Lookup("LOC_CITY_BANNER_WEAPON_UNAVAILABLE"));
				end
			else
				self.m_Instance.MISSILE_BASECountLabel:SetText("0");
				self.m_Instance.MISSILE_BASEBombButtonBackground:SetHide(true);
			end	
		elseif (entry.WeaponType == "WMD_CASTLE_BRAVO") then
			local count = playerWMDs:GetWeaponCount(entry.Index);
			if (count > 0) then
				self.m_Instance.CASTLE_BRAVOCountLabel:SetText(count);
				self.m_Instance.CASTLE_BRAVOButtonBackground:SetHide(false);
				-- Check if we're able to fire
				local bSiloCanFire:boolean = false;
				if( pCity ~= nil ) then
					local tParameters = {};
				...
				-- Update button state and tooltip
				if( bSiloCanFire) then
					self.m_Instance.CASTLE_BRAVOBombButton:SetDisabled(false);
					self.m_Instance.CASTLE_BRAVOBombButton:SetToolTipString(Locale.Lookup("LOC_WMD_CASTLE_BRAVO_NAME"));
				else
					self.m_Instance.CASTLE_BRAVOBombButton:SetDisabled(true);
					self.m_Instance.CASTLE_BRAVOBombButton:SetToolTipString(Locale.Lookup("LOC_CITY_BANNER_WEAPON_UNAVAILABLE"));
				end
			else
				self.m_Instance.CASTLE_BRAVOCountLabel:SetText("0");
				self.m_Instance.CASTLE_BRAVOButtonBackground:SetHide(true);
			end		
		elseif (entry.WeaponType == "WMD_KUZKINE_MET") then
			local count = playerWMDs:GetWeaponCount(entry.Index);
			if (count > 0) then
				self.m_Instance.KUZKINE_METCountLabel:SetText(count);
				self.m_Instance.KUZKINE_METButtonBackground:SetHide(false);
				-- Check if we're able to fire
				local bSiloCanFire:boolean = false;
				if( pCity ~= nil ) then
					local tParameters = {};
				...
				-- Update button state and tooltip
				if( bSiloCanFire) then
					self.m_Instance.KUZKINE_METBombButton:SetDisabled(false);
					self.m_Instance.KUZKINE_METBombButton:SetToolTipString(Locale.Lookup("LOC_WMD_KUZKINE_MET_NAME"));
				else
					self.m_Instance.KUZKINE_METBombButton:SetDisabled(true);
					self.m_Instance.KUZKINE_METBombButton:SetToolTipString(Locale.Lookup("LOC_CITY_BANNER_WEAPON_UNAVAILABLE"));
				end
			else
				self.m_Instance.KUZKINE_METCountLabel:SetText("0");
				self.m_Instance.KUZKINE_METButtonBackground:SetHide(true);
			end					
		end
	end
end
...
function CityBanner:CreateEncampmentBanner()
	...
	if (pDistrict ~= nil) then
		...
		-- GA:Set the harbors to proper harbor icons, the condition domain sea should include all harbors and UDs...
		if kDistrictDef ~= nil and kDistrictDef.MilitaryDomain == "DOMAIN_SEA" then
			self.m_Instance.EncampmentFontIcon:SetText("[Icon_DISTRICT_HARBOR]");
		end
...
function CityBanner:UpdateDetails()
	...
	if pCity and pDistrict then
		...
		local iCurrentEra =	Game.GetEras():GetCurrentEra();--GA											 
